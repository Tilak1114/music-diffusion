        ...
        if self.trainer.global_rank == 0:
            ...
            if current_epoch % 5 == 0:
                ...
                for batch in val_dataloader:
                    ...
                    for i in range(len(latent_paths[0])):
                        concatenated_gt_waves = []
                        for j in range(len(latent_paths)):
                            concatenated_gt_waves.append(
                                self.v_sampler.latents_to_wave(
                                    torch.load(latent_paths[j][i])
                                )
                            )
                        concatenated_wav = np.concatenate(
                            concatenated_gt_waves, axis=1)
                        sf.write(os.path.join(
                            ground_truth_dir, f"{save_file_names[i]}.wav"),
                            concatenated_wav.squeeze(), samplerate=16000)
                        # Process the prompt embedding
                        prompt_emb = torch.load(prompt_emb_paths[i])
                        loaded_text_embs.append(prompt_emb)
                    ...
                    split_latents = [gen_latents[:, :, i, :, :]
                                     for i in range(sequence_length)]

                    for i, split_latent in enumerate(split_latents):
                        gen_wavs = self.v_sampler.latents_to_wave(split_latent)
                        concatenated_gen_wave = np.concatenate(gen_wavs, axis=0)

                        sf.write(os.path.join(generated_output_dir,
                                              f"{save_file_names[i]}.wav"),
                                 concatenated_gen_wave.squeeze(), 
                                 samplerate=16000
                                 )
