...
class LatentMusicDiffusionModel(pl.LightningModule):
    ...
    def on_train_start(self):
        if self.trainer.global_rank == 0:
            ...
            mlflow.set_tracking_uri("file:///data/tilak/projects/music-diffusion/mlflow")
            mlflow.set_experiment(self.config['name'])

            config = self.config

            self.mlflow_run = mlflow.start_run(run_name=self.config['name'], description=self.config['description'])
            ...
            mlflow.log_param("use_text_conditioning", config["module_config"]["unet"]["use_text_conditioning"])
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.global_rank == 0 and not self.trainer.sanity_checking:
            mlflow.log_metric('val_loss', val_loss.item(), step=self.current_epoch)
    ...
    def on_train_epoch_end(self, outputs=None):
        ...
        if self.trainer.global_rank == 0:
            ...
            if current_epoch % 5 == 0:
                self.save_epoch_results(current_epoch)

    def save_epoch_results(self, current_epoch, full_val_data = False):
        val_dataloader = self.val_dataloader()
        sample_batch = next(iter(val_dataloader))
        audio_file_names, latent_paths, video_embedding_paths, prompt_embedding_paths = sample_batch
        ...
        mlflow.log_artifacts(
            generated_output_dir, artifact_path=f"epoch_{current_epoch}/generated_outputs")
